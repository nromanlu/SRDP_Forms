 ...
 * send-report.php
 *
 * Receives the generated Einsatzrapport PDF from einsatzrapport.html and emails it
 ...
 * buildEditUrl() in einsatzrapport.html) so that opening it re-fills the form,
 ...
 *
 * Deploy this file in the SAME folder as einsatzrapport.html so the relative
 ...
 *
 * Identical in every respect to Pikettrapport/send-report.php except for the
 * default subject/filename fallback text below - kept as a separate copy
 * (rather than a shared include) so each report folder deploys standalone.
 */
...
$subject        = isset($_POST['subject']) ? trim($_POST['subject']) : 'Einsatzrapport';
$bodyText       = isset($_POST['body']) ? trim($_POST['body']) : '';
$attachmentName = isset($_POST['filename']) ? basename($_POST['filename']) : 'Einsatzrapport.pdf';
