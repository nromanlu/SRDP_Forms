 ...
 *
 * Two optional additional attachments ("Tankquittung" fuel receipt photos, one
 * per boat - field names "receipt_biene" and "receipt_weidlig") are accepted the
 * same way as the PDF: read straight out of PHP's own upload temp files into the
 * outgoing email and never written anywhere persistent (no move_uploaded_file()
 * into a folder, unlike the signature tool's PIN-protected signatures/ storage) -
 * once this script returns, nothing of them remains on the server. Both are
 * entirely optional and independent; a request with neither sends exactly like
 * before, and either, both, or neither can be present on any given request.
 *
...
// ---------------------------------------------------------------------------
// Optional "Tankquittung" fuel receipt photos - one per boat (Biene III /
// FW-Weidlig), each entirely optional and independent. A request without
// either (the normal case whenever nobody bunkered fuel this callout) sends
// exactly like before. Read from PHP's own upload temp files only - never
// moved to a persistent folder, so there's nothing left on the server once
// this script's response is sent, matching how the PDF itself is already
// handled.
// ---------------------------------------------------------------------------
$RECEIPT_FIELDS = [
    'receipt_biene'   => 'Tankquittung_Biene-III',
    'receipt_weidlig' => 'Tankquittung_FW-Weidlig',
];
$allowedReceiptTypes = ['image/jpeg' => 'jpg', 'image/png' => 'png'];
$receiptAttachments = []; // each: ['data' => ..., 'contentType' => ..., 'filename' => ...]

foreach ($RECEIPT_FIELDS as $fieldName => $baseFilename) {
    if (!isset($_FILES[$fieldName]) || $_FILES[$fieldName]['error'] !== UPLOAD_ERR_OK) {
        continue;
    }

    if ($_FILES[$fieldName]['size'] > $MAX_RECEIPT_MB * 1024 * 1024) {
    ...
    if (function_exists('finfo_open')) {
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
        $receiptType = finfo_file($finfo, $_FILES[$fieldName]['tmp_name']);
        finfo_close($finfo);
    }
    ...
    $receiptData = file_get_contents($_FILES[$fieldName]['tmp_name']);
    ...
    $receiptAttachments[] = [
        ...
        'filename'    => $baseFilename . '.' . $allowedReceiptTypes[$receiptType],
    ];
}
...
// --- Tankquittung attachment(s) (optional - only present if any receipt was sent) ---
$receiptParts = '';
foreach ($receiptAttachments as $receipt) {
    $receiptParts .= "--{$mixedBoundary}\r\n";
    $receiptParts .= "Content-Type: {$receipt['contentType']}; name=\"{$receipt['filename']}\"\r\n";
    $receiptParts .= "Content-Transfer-Encoding: base64\r\n";
    $receiptParts .= "Content-Disposition: attachment; filename=\"{$receipt['filename']}\"\r\n\r\n";
    $receiptParts .= chunk_split(base64_encode($receipt['data'])) . "\r\n";
}

$message = $alternative . $attachment . $receiptParts . "--{$mixedBoundary}--";
